death_save = {
	...
	uncast = function(player)
		...
		if player:canPK(player) then
			...
			if ((attacker.m == 33 or attacker.m == 3011 or attacker.m == 3017) and attacker.blType == BL_PC) then
				attacker.registry["carnageKillCount"] = attacker.registry["carnageKillCount"] + 1
				player.registry["carnageDeathCount"] = player.registry["carnageDeathCount"] + 1
			end
			if ((attacker.m == 3015) and attacker.blType == BL_PC) then
				attacker.registry["blKillCount"] = attacker.registry["blKillCount"] + 1
				player.registry["blDeathCount"] = player.registry["blDeathCount"] + 1
			end
		...
		if player.mapTitle ~= "Kugnae" and player.mapTitle ~= "Buya" and player.mapTitle ~= "Nagnang" and player.mapTitle ~= "Mythic Nexus" and player.mapTitle ~= "KaMing's Encampment" and player.mapTitle ~= "Blank Kingdom" then
			if (player.m < 59000 or player.m > 65000) and (player.m ~= 32103) then
				-- not include instances
				player:deathDropGold()
				player:deathExpLoss(0.5)
				player:deathDuraLoss()
				player:deathPileDrop()
			elseif player.m == 32103 then
				broadcast(32103, ""..player.name.." got borked in Dogwood! Gobble gobble!")
		...
		for i = 1, #pc do
			if attacker == nil then
				...
				if attacker.yname == "angry_elf" or attacker.yname == "angrier_elf" or attacker.yname == "angriest_elf" or attacker.yname == "grumpy_elf" then
					pc[i]:msg(
						4,
						"[DEATH]: " .. player.name .. " was killed by " .. attacker.gfxName .. " at " .. player.mapTitle,
						pc[i].ID
					)
				else
					pc[i]:msg(
