...
paralyze_spellsword = {
	cast = function(player, target)
		...
		if (target.blType == BL_MOB) then
			...
			player:sendStatus()
			player:playSound(70)
--			player:sendMinitext("Your blade paralyzes your foe.")
			target:setDuration("paralyze_spellsword", duration)
			target:sendAnimation(403, 0)
			target.paralyzed = true
		end
	end,
