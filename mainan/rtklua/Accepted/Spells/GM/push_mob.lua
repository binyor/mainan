	...
	cast = function(player)
		...
		if target ~= nil then
			...
			if objCheck == 0 and passCheck == 0 and not warpCheck and #blockCheck == 0 and targetX >= 0 and targetX < maxX and targetY >= 0 and targetY < maxY then
				...
				target:sendMinitext(player.name .. " menggunakan Push spell ke kamu.")
				return
			else
				if player.side == 0 then
					targetY = targetY - 1
				...
				if objCheck == 0 and passCheck == 0 and not warpCheck and #blockCheck == 0 and targetX >= 0 and targetX < maxX and targetY >= 0 and targetY < maxY then
					target:warp(targetM, targetX, targetY)
					target:sendAnimation(10, 25)
					player:sendMinitext("Anda menggunakan Push.")
					target:sendMinitext(player.name .. " menggunakan Push spell ke kamu.")
					return
				else
					player:sendMinitext("Kamu tidak bisa mendorongnya.")
				end
			end
		end
	end,
