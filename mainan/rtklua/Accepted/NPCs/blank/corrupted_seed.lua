corrupted_seed = {
	on_drop = function(player, item)
		...
		if (player.x >= 28 and player.x <= 32 and player.y >= 32 and player.y <= 30) and player.quest["blackmage_trial_of_repudiation"] == 1 then
			player:removeItem("corrupted_seed", 1)
			player.quest["blackmage_trial_complete"] = 1
			player.registry["seedplanted"] = player.registry["seedplanted"] + 1
