...
KanNpc = {
	click = async(function(player, npc)
		...
		local opts = {
			"btkoin account information",
			"Buy btkoin Certificates",
			"Crafting bags",
			"Cosmetics",
			"Browse Other",
			"War Paint"
		...
		local choice = player:menuString(
			"Hello and welcome to the btkoin shop! How can I help you today?",
			opts
		)

		if choice == "btkoin account information" then
			KanNpc.getKanAccountInfo(player)
		elseif choice == "Buy btkoin Certificates" then
			local items = {
				"kan_certificate_500",
			...
			if confirm == 1 then
				if player.registry[kanRegistry] < expPack.price then
					player:dialogSeq(
						{
							t,
							"Unfortunately, you do not have enough btkoin to complete this purchase."
						},
						0
					)
					return
		...
		elseif choice == "Cosmetics" then
			local subChoices = {"Equipment", "Faces", "Hair Dyes"}
		...
		elseif choice == "Browse Mounts" then
			...
			while str == "buff" do
				...
				local optsChoice = player:menuString(
					"<b>Mount: " .. mount.name .. "\n\nPrice: " .. Tools.formatNumber(mount.price) .. " btkoin\n\nIs this style acceptable?",
					opts
				)

				if optsChoice == "Buy mount" then
					local confirm = player:menuString(
						"Ah, I see you have found a suitable mount.\n\nThe price will be: " .. Tools.formatNumber(mount.price) .. " tbkoin. Would you like to buy?",
						{"Yes, I want to buy.", "No, nevermind."}
					)
		...
		elseif choice == "Random Mount Boxes" then
			...
			if subChoice == 1 then
				...
				local confirm = player:menuSeq(
					"This " .. Item(items[choice]).name .. " box will cost " .. Tools.formatNumber(price) .. " btkoin. Do you wish to buy it?",
					{"Yes", "No"},
					{}
				)
				...
				if confirm == 1 then
					if player.registry[kanRegistry] < price then
						player:dialogSeq(
							{
								t,
								"Sorry but you do not have enough btkoin for this purchase."
							},
							0
						)
						return
	...
	getKanAccountInfo = function(player)
		local donate = player:menuSeq(
			"btkoin balance: " .. Tools.formatNumber(player.registry[kanRegistry]) .. "\n\nWould you like to purchase more btkoin? (nmail micdog in-game)",
			{"Yes", "No"},
			{}
		)
		...
		if donate == 1 then
			...
			local confirmDonate = player:menuSeq(
				"The gods demand a significant tribute in exchange for btkoin. Are you sure?",
				{"Buy " .. Tools.formatNumber(kanAmount) .. " btkoin for " .. Tools.formatNumber(kanPrice) .. " coins", "Maybe next time"},
				{}
			)
			...
			player:sendMinitext(Tools.formatNumber(kanAmount) .. " btkoin has been added to your account.")
			player:forceSave()
		end
	end,
