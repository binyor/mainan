...
MessengerNpc = {
	click = async(function(player, npc)
		...
		if(player.registry["bardq"] == 7) then
			table.insert(opts, "Seong sent me")
		end
		if(player.registry["bardq"] == 9) then
			player:addItem("barrel_of_ink",1)
			player.registry["bardq"] = 10
			player:dialogSeq({"Oh you're back, a little late for my taste. But whatever, just take it and leave."}, 1)
		end
		...
		if menu == "Seong sent me" then
			player:dialogSeq({"Oh they did? Well, you've come at the right time. I'm going to need you to do a small task beforehand.", "I'm running short on the ingredients to make the ink. Gather me some Amethyst, Topaz, and Mica. A half dozen each should be fine.","Oh, and I'll need a brace of sonhi pipes for uh... reasons. **You notice he can't bare to look you in the eye**"},1)
			if(player:hasItem("amethyst",6) == true and player:hasItem("mica",6) == true and player:hasItem("topaz",6) == true and player:hasItem("sonhi_pipe",2) == true) then
				player:removeItem("amethyst",6)
				player:removeItem("mica",6)
				player:removeItem("topaz",6)
				player:removeItem("sonhi_pipe",2)
				player.registry["bardq"] = 8
				player:dialogSeq({"THANK YOU. THANK YOU. THANK YOU. THANK YOU. THANK YOU. THANK YOU.","......","Oh... Uh, right, the ink... While you wait for me to finish up, you should head back to Seong."},1)
			end
		end
