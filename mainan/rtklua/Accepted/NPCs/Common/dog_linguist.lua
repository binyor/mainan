		...
		if player:hasLegend("dog_linguist") then
			if speech == "secret" then
				...
				elseif npc.name == "Hunting dog" and (player.class == 1 or player.class == 6) then
					...
					if player.quest["dog_linguist_subpath"] == 0 then
--[[ 						local choice = player:menuSeq("Indeed, I could teach you much, but first, you would have to commit to NEVER join a subpath.  Do you commit?",{"Yes, the subpaths hold nothing for me.","No, I have not yet chosen my destiny."},{})
						player:talk(0,""..choice)
						if choice == "Yes, the subpaths hold nothing for me." then ]]
							player.quest["dog_linguist_subpath"] = 1
--[[ 						else
							return
						end ]]
