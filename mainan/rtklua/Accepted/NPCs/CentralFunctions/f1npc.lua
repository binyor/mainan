F1Npc = {
	click = async(function(player, npc)
		...
		if choice == "Set Pin Number" then
			--if player.gmLevel > 0 then player:sendMinitext("Change your pin via SQL.") return end
			...
			local test = 0
			while test == 0 do
				local input = player:inputNumberCheck(player:inputSeq("Enter your new Pin.\n\nMust be 6 or more digits.","","",{},{}))
				if input == nil then return end
				if (string.len(input) > 5) then
					test = 1
					input = player.id..input..now
					input = sha.sha256(input)
					player.registryString["twofactor_key"] = input
					player.registryString["twofactor_time"] = now
					player:msg(0,"[PIN]: Your new pin has been updated!",player.id)
					player:msg(0,"[PIN]: You will need to enter this every time you log in. If you want to turn it off, contact a GM.",player.id)
				end
			end
		end
		...
		if choice == "Silver Thread" then
			...
			elseif player.country == 1 or player.country == 4 or player.country == 3 then
				--kugnae
				warpChoice = player:menuString(
					"Which Shaman would you like to visit?",
					{
		...
		elseif choice == "Toggles" then
			...
			while s ~= "nothing" do
				...
				elseif menuChoice == "Misc Toggles" then
					local toggles = {"See Warps", "Disable Experience Gain", "Display Auto-Swing timer"}
					local currentStatus = {
						player.registry["see_warps"],
						player.registry["disableExperienceGain"],
						player.registry["display_auto_swing"]
					}
					local toggleS = {}
					local status
					...
					if toggleChoice == 1 then
						...
						elseif currentStatus[toggleChoice] == 1 then
					...
					elseif toggleChoice == 3 then
						if currentStatus[toggleChoice] == 0 then
							player.registry["display_auto_swing"] = 1
							player:sendMinitext("Display Auto-Swing timer: ON")
						elseif currentStatus[toggleChoice] == 1 then
							player.registry["display_auto_swing"] = 0
							player:sendMinitext("Display Auto-Swing timer: OFF")
						end
					end
				end
			end
		...
		elseif choice == "Character Stats" then
			...
			player:popUp(string)
		elseif choice == "Gleaming Sword" then
			player:gleamingSword()
		elseif choice == "My character webpage" then
			...
			elseif choice == "Help" then
				player:dialogSeq(
					{
						t,
						"This feature will allow you to set up your own specific options for a RTK user page (https://users.Three Kingdoms.com)",
		...
		elseif choice == "AFK Message" then
			...
			player:updateState()
		elseif choice == "btkoin account information" then
