...
YonNpc = {
	click = async(function(player, npc)
		...
		if player.registry["hunterq"] == 1 then
			table.insert(opts, "Artemis sent me")
		end
		...
		elseif choice == "Artemis sent me" then
			if((player:hasItem("wool",100) == true) and (player:hasItem("cloth",50) == true) and (player:hasItem("bear_fur",12) == true) and (player:hasItem("splendid_tiger_pelt",3) == true) and (player:hasItem("antler",1) == true)) then
				player:removeItem("wool",100)
				player:removeItem("cloth",50)
				player:removeItem("bear_fur",12)
				player:removeItem("splendid_tiger_pelt",3)
				player:removeItem("antler",1)
				player.registry["hunterq"] = 2
				player:dialogSeq({"Oh, wonderful job sweetie! You'll be warm as a kitten tonight.",0})
			else
				player:dialogSeq({"Oh, Hunter Artemis sent you?? That means he likes you if he doesn't want you freezing.","Of course I'll make you a coat! The harsh winters require a thick layer, though..","I'll need a stack of Wool, half of Cloth, a dozen or so bear fur,","three VERY NICE tiger skins, and an antler!",0})		
			end
		end
