...
ZephyrNpc = {
	click = async(function(player, npc)
		...
		if(player.registry["bardq"] == 8) then
			table.insert(opts, "A skilled quill?")
		end
		...
		elseif menu == "A skilled quill?" then
			player.registry["bardq"] = 9
			player:dialogSeq({"Welcome to my mountain. I can only assume, Seong sent you?","Well as it just so happens, I was going to visit him in a couple days.","It seems you've wasted a journey, but atleast I've got to meet his new protege.","Please be safe on your way back. I'll hope to see you soon."},1)
	...
	onSayClick = async(function(player, npc)
		...
		if speech == "magical book" or speech == "magic book" or speech == "magical scribe's book" and player.registry["wiz_trial"] == 1 then
			if type(player:hasItem("scribes_book", 1)) == "boolean"  and type(player:hasItem("moon_paper", 2)) == "boolean" then
				player:removeItem("scribes_book", 1)
				player:removeItem("moon_paper", 2)
				player:addItem("wiz_magical_scribes_book", 1)
				player:dialogSeq({t,"Ahh.. it's nice to have moon paper around.",
				"Here is your magical scribe's book, please take good care of it."}, 1)	
			else
				player:dialogSeq({t,"Ahh.. the wizards must have sent you.",
				"If you would like a magical scribe's book, you will first need to bring me 2 paper from the stars and a regular scribe's book.",
				"Once you have these items let me know and I will help you craft it."}, 1)
			end
		end
