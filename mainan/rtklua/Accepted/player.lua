...
function Player.removeHealthWithoutDamageNumbers(player, amount, type)
	if (player:hasDuration("harden_body_poet")) or (player:hasDuration("deaths_guard_poet")) or	(player:hasDuration("lifes_protection_poet")) or (player:hasDuration("body_of_alignment_poet")) or (player:hasDuration("botcheck_hb")) then
		return
	end
	local temp_health = 0
...
function Player.showClanBankDeposit(player, npc)
	...
	if dItem.dura ~= dItem.maxDura then
		player:sendMinitext("Your item has to be at full durability to deposit.")
		return false
	end
...
function Player.repairExtend(player)
	...
	if choice ~= nil and choice.dura < choice.maxDura then
		...
		cost = math.ceil(((choice.price / choice.maxDura) * (choice.maxDura - choice.dura)) * 0.2)
...
function Player.repairAll(player, npc)
	...
	if (playerFaceAcc ~= nil) then
		if (playerFaceAcc.repairable == 1) then
			if (playerFaceAcc.dura < playerFaceAcc.maxDura) then
				playerFaceAccCost = math.ceil(((playerFaceAcc.price / playerFaceAcc.maxDura) * (playerFaceAcc.maxDura - playerFaceAcc.dura)) * 0.2)
			end
		else
	...
	if (playerHelm ~= nil) then
		if (playerHelm.repairable == 1) then
			if (playerHelm.dura < playerHelm.maxDura) then
				playerHelmCost = math.ceil(((playerHelm.price / playerHelm.maxDura) * (playerHelm.maxDura - playerHelm.dura)) * 0.2)
			end
		else
	...
	if (playerCrown ~= nil) then
		if (playerCrown.repairable == 1) then
			if (playerCrown.dura < playerCrown.maxDura) then
				playerCrownCost = math.ceil(((playerCrown.price / playerCrown.maxDura) * (playerCrown.maxDura - playerCrown.dura)) * 0.2)
			end
		else
	...
	if (playerWeapon ~= nil) then
		if (playerWeapon.repairable == 1) then
			if (playerWeapon.dura < playerWeapon.maxDura) then
				playerWeaponCost = math.ceil(((playerWeapon.price / playerWeapon.maxDura) * (playerWeapon.maxDura - playerWeapon.dura)) * 0.2)
			end
		else
	...
	if (playerArmor ~= nil) then
		if (playerArmor.repairable == 1) then
			if (playerArmor.dura < playerArmor.maxDura) then
				playerArmorCost = math.ceil(((playerArmor.price / playerArmor.maxDura) * (playerArmor.maxDura - playerArmor.dura)) * 0.2)
			end
		else
	...
	if (playerShield ~= nil) then
		if (playerShield.repairable == 1) then
			if (playerShield.dura < playerShield.maxDura) then
				playerShieldCost = math.ceil(((playerShield.price / playerShield.maxDura) * (playerShield.maxDura - playerShield.dura)) * 0.2)
			end
		else
	...
	if (playerLeft ~= nil) then
		if (playerLeft.repairable == 1) then
			if (playerLeft.dura < playerLeft.maxDura) then
				playerLeftCost = math.ceil(((playerLeft.price / playerLeft.maxDura) * (playerLeft.maxDura - playerLeft.dura)) * 0.2)
			end
		else
	...
	if (playerRight ~= nil) then
		if (playerRight.repairable == 1) then
			if (playerRight.dura < playerRight.maxDura) then
				playerRightCost = math.ceil(((playerRight.price / playerRight.maxDura) * (playerRight.maxDura - playerRight.dura)) * 0.2)
			end
		else
	...
	if (playerMantle ~= nil) then
		if (playerMantle.repairable == 1) then
			if (playerMantle.dura < playerMantle.maxDura) then
				playerMantleCost = math.ceil(((playerMantle.price / playerMantle.maxDura) * (playerMantle.maxDura - playerMantle.dura)) * 0.2)
			end
		else
	...
	if (playerSubLeft ~= nil) then
		if (playerSubLeft.repairable == 1) then
			if (playerSubLeft.dura < playerSubLeft.maxDura) then
				playerSubLeftCost = math.ceil(((playerSubLeft.price / playerSubLeft.maxDura) * (playerSubLeft.maxDura - playerSubLeft.dura)) * 0.2)
			end
		else
	...
	if (playerSubRight ~= nil) then
		if (playerSubRight.repairable == 1) then
			if (playerSubRight.dura < playerSubRight.maxDura) then
				playerSubRightCost = math.ceil(((playerSubRight.price / playerSubRight.maxDura) * (playerSubRight.maxDura - playerSubRight.dura)) * 0.2)
			end
		else
	...
	if (playerNecklace ~= nil) then
		if (playerNecklace.repairable == 1) then
			if (playerNecklace.dura < playerNecklace.maxDura) then
				playerNecklaceCost = math.ceil(((playerNecklace.price / playerNecklace.maxDura) * (playerNecklace.maxDura - playerNecklace.dura)) * 0.2)
			end
		else
	...
	if (playerBoots ~= nil) then
		if (playerBoots.repairable == 1) then
			if (playerBoots.dura < playerBoots.maxDura) then
				playerBootsCost = math.ceil(((playerBoots.price / playerBoots.maxDura) * (playerBoots.maxDura - playerBoots.dura)) * 0.2)
			end
		else
	...
	if (playerCoat ~= nil) then
		if (playerCoat.repairable == 1) then
			if (playerCoat.dura < playerCoat.maxDura) then
				playerCoatCost = math.ceil(((playerCoat.price / playerCoat.maxDura) * (playerCoat.maxDura - playerCoat.dura)) * 0.2)
			end
		else
	...
	if #inventoryItems > 0 then
		for i = 1, #inventoryItems do
			inventoryItemsCost = inventoryItemsCost + math.ceil(((inventoryItems[i].price / inventoryItems[i].maxDura) * (inventoryItems[i].maxDura - inventoryItems[i].dura)) * 0.2)
		end
	end
			...
			if (invItem ~= nil and invItem.type > 2 and invItem.type < 17) then
				if invItem.repairable == 1 then
					if (invItem.dura < invItem.maxDura) then
						invCost = math.ceil(((invItem.price / invItem.maxDura) * (invItem.maxDura - invItem.dura)) * 0.2)
...
function Player.countryName(player, country)
	...
	if tonumber(country) == 4 then
		return "Blank Kingdom"
	end
end
...
Player.returnToInn = function(player)
	...
	--[[countries   0: neutral, 1: kug, 2: buya, 3: nagnang, 4: blank]]
	--
	...
	elseif (country == 4) then
		...
		if random == 1 then
			player:warp(753, 19, 12)
		elseif random == 2 then
			player:warp(753, 20, 12)
		end

		-- Ginger Tavern (now su tavern)
	end
end
...
Player.returnToClan = function(player)
	...
	if clanName == "Immortals" then
		m, x, y = 600, math.random(10, 12), math.random(10, 12)
	elseif clanName == "Koguryo Army" then
		return
	elseif clanName == "Buya Army" then
		return
	elseif clanName == "Cartel" then
		m, x, y = 5066, math.random(14, 15), math.random(15,16)
	elseif clanName == "VIKINGS" then
		m, x, y = 5069, math.random(14, 16), 3
	elseif clanName == "Zen" then
		m, x, y = 5071, math.random(15, 15), math.random(18,19)
...
function Player.regen(player)
	...
	--if (player.timerTick % 25 == 0 and (string.match(player.mapTitle, "Inn") ~= nil or string.match(player.mapTitle,"Tavern") ~= nil)) or player.timerTick % 50 == 0 then
		...
		if string.match(player.mapTitle, "Inn") ~= nil or string.match(player.mapTitle,"Tavern") ~= nil and math.random(1, 20) == 1 then
...
--[[
Player.gui = function(player, text)
	local texts = {}
	if player.registryString["gui"] == "" then
		--text = text
	else
		string.find(\n)
		player.registryString["gui"]
	end
	player:guitext(text)
end
]]--

function Player.carnageSpell(player)
	if player.m >= 3000 and player.m <= 3019 then
		player:sendMinitext("This spell is forbidden in carnages")
		return true
	else
		return false
	end
