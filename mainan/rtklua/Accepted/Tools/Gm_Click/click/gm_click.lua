gm_click = {
	menu = async(function(player, target, npc)
		...
		if menu ~= nil then
			...
			if menu == "Name Color" then
				gm_click.name_color(player, target)
				return
			end
	...
	totem_subpath_grade = function(player, target, npc)
		...
		elseif menu == 2 then
			...
			if target.baseClass == 1 then
				spec = {"None", "Chung ryong", "Barbarian", "Chongun", "Do","Knight","Samurai","Spellsword"}
			end
			if target.baseClass == 2 then
				spec = {"None", "Baekho", "Merchant", "Ranger", "Spy","Hunter","Jester","Pirate"}
			end
			if target.baseClass == 3 then
				spec = {"None", "Ju jak", "Diviner", "Geomancer", "Shaman","Bloodmage","Summoner","Wizard"}
			end
			if target.baseClass == 4 then
				spec = {"None", "Hyun moo", "Druid", "Monk", "Muse","Shape-shifter","Bard","White Mage"}
			end
	...
	bug_legend = function(player, gm)
		...
		if reg > 0 then
			...
			player:addLegend(
				"Recognized for finding " .. player.registry["bugs_found"] .. " bugs in Three Kingdoms",
				"bugfinder",
				213,
				244
			)
			...
			player:addLegend(
				"Recognized for finding 1 bug in Three Kingdoms",
				"bugfinder",
				213,
				244
			)
	...
	gmassist_legend = function(player, gm)
		...
		if reg > 0 then
			...
			player:addLegend(
				"Assisted the Three Kingdoms GMs " .. player.registry["gm_assist"] .. " times",
				"gmassist",
				145,
				226
			)
			...
			player:addLegend(
				"Assisted the Three Kingdoms GMs 1 time",
				"gmassist",
				145,
				226
			)
	...
	name_color = function(player, target)
		local colors = {}
		for i = 1, 255 do
			table.insert(colors,i)
		end
		local pickedcolor = player:menuString("What do you want to set "..target.name.."'s color to?",colors)
		if pickedcolor >= 0 and pickedcolor <= 255 then
			target.registry["title_color"] = pickedcolor
			target.gfxTitleColor = pickedcolor
			target.gfxClone = 1
			target.gfxDye = 1
			target.gfxName = target.name
			target.gfxClone = 0
			target:refresh()
		end
	end,
