onScriptedTilesQuest = async(function(player)
	...
	if (player.mapTitle == "Cave") then
		if (player.x >= 4 and player.x <= 7 and player.y >=10 and player.y <= 11) then
			if(player.registry["sstrial"] == 4) then
				if(player.registry["sstrialspawn"] == 0) then
					core:spawn(811, 5, 4, 1, 522)
					player:addThreatGeneral(100)
					player.registry["sstrialspawn"] = 1
				end
			end
		end
	end
