login = function(player)
	...
	if player.registry["title_color"] ~= 0 then
		player.gfxClone = 1
		player.gfxDye = 1
		player.gfxName = player.name
		player.gfxTitleColor = player.registry["title_color"]
		player.gfxClone = 0
		player:refresh()
	end
	...
	if player.registry["botcheck"] ~= 0 then
		player:setDuration("botcheck",0)
		return
	end

	if (player:hasDuration("botcheck_hb") == true) then
		player:setDuration("botcheck_hb",0)
	end
	...
	if (player.m == 3010 or player.m == 3011 or player.m == 33 or player.m == 3017 or player.m == 3015) then
	...
	local twofac = player.registryString["twofactor_key"] -- 2FA
	if twofac ~= "" then
		player:setDuration("twofactor", 5000)
		player:setDuration("twofactorauth",35000)
	else
		if player.gmLevel > 0 then
			print("[PIN]: Disconnected "..player.name.." | Reason: GM w/o 2FA")
			--player:disconnect()
		else
			player:msg(0,"Check your F1 menu to enable your Pin (2FA) on login.",player.ID)
		end
	end
...
mapChange = function(player, m, x, y)
--	print(player.name .. " warping to map(" .. m .. ") at " .. x .. "/" .. y)
end
