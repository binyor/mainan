carnages = {
	init = function(eventid)
		...
		if core.gameRegistry["minigameEventId"] == 22 then
			core.gameRegistry["carnageMaxHealth"] = 0
			core.gameRegistry["carnageMaxMagic"] = 0
		end
		if core.gameRegistry["minigameEventId"] == 56 then
			core.gameRegistry["carnageMaxHealth"] = 0
			core.gameRegistry["carnageMaxMagic"] = 0
		end
		...
		setMapRegistry(3016, "maxHealth", core.gameRegistry["carnageMaxHealth"])
		setMapRegistry(3016, "maxMagic", core.gameRegistry["carnageMaxMagic"])

		setMapRegistry(3018, "maxHealth", core.gameRegistry["carnageMaxHealth"])
		setMapRegistry(3018, "maxMagic", core.gameRegistry["carnageMaxMagic"])
	...
	closeCarnageDoors = function()
		...
		local obj = getObject(m, 10, 2)
		setObject(m, 10, 2, (obj - 18))
		local obj2 = getObject(m, 11, 2)
		setObject(m, 11, 2, (obj2 - 18))
	end,
	...
	timer = function(eventid)
		local maps = {3011, 3010, 33, 3017, 3015, 3016, 3018}
		...
		if (core.gameRegistry["carnageState"] == 8) then
			if (core.gameRegistry["carnageBlackScore"] < 2 and core.gameRegistry["carnageRedScore"] < 2 and core.gameRegistry["carnageWhiteScore"] < 2 and core.gameRegistry["carnageBlueScore"] < 2) then
				...
				if (#players <= 100) then
					...
					if core.gameRegistry["minigameEventId"] == 22 then
						core.gameRegistry["carnageMap"] = 3015
					end
					if mapChoice == 1 then
						core.gameregistry["carnageMap"]= 3017
					elseif mapChoice == 2 then
						core.gameRegistry["carnageMap"] = 3016
					elseif mapChoice == 3 then
						core.gameRegistry["carnageMap"] = 3018
					end
					if core.gameRegistry["minigameEventId"] == 22 then
						core.gameRegistry["carnageMap"] = 3015
					end
		...
		if (core.gameRegistry["carnageState"] == 11) then
			if (os.time() > core.gameRegistry["carnageStart"] + 10) then
				...
				local npcBlocks = core:getObjectsInMap(core.gameRegistry["carnageMap"],BL_NPC)
				local npcsToRemove = {}
				...
				if #npcsToRemove > 0 then
					for i = 1, #npcsToRemove do
						local items = core:getObjectsInCellWithTraps(
							npcsToRemove[i].m,
							npcsToRemove[i].x,
							npcsToRemove[i].y,
							BL_ITEM
...
--[[
	...
	chooseMap = function()
		if core.gameRegistry["carnageMap"] == 0 then
			local carnageMaps = {31, 3009, 3011, 3016, 3017, 3018}
		...
		if m == 3016 then
			for z = 1, #players do
				if (players[z].state == 0) then
					...
					players[z].magic = players[z].maxMagic
					if (players[z].gfxDye == 63) then
						players[z]:warp(m, 11, 9)
					elseif (players[z].gfxDye == 60) then
						players[z]:warp(m, 15, 13)
					elseif (players[z].gfxDye == 61) then
						players[z]:warp(m, 11, 13)
					elseif (players[z].gfxDye == 66) then
						players[z]:warp(m, 15, 9)
					end
				end
			end
		end
		if m == 3018 then
			for z = 1, #players do
				if (players[z].state == 0) then
					...
					players[z].magic = players[z].maxMagic
					if (players[z].gfxDye == 63) then
						players[z]:warp(m, 28, 5)
					elseif (players[z].gfxDye == 60) then
						players[z]:warp(m, 32, 9)
					elseif (players[z].gfxDye == 61) then
						players[z]:warp(m, 28, 9)
					elseif (players[z].gfxDye == 66) then
						players[z]:warp(m, 32, 5)
					end
				end
			end
		end
	...
	setCircleBarricade = function()
		...
		if m == 3016 then
			for x = 10, 16 do
				carnages.dropBarricade(m, x, 8)
				carnages.dropBarricade(m, x, 11)
				carnages.dropBarricade(m, x, 14)
			end
			for y = 8, 14 do
				carnages.dropBarricade(m, 10, y)
				carnages.dropBarricade(m, 13, y)
				carnages.dropBarricade(m, 16, y)
			end
		end
		if m == 3018 then
			for x = 27, 33 do
				carnages.dropBarricade(m, x, 4)
				carnages.dropBarricade(m, x, 7)
				carnages.dropBarricade(m, x, 10)
			end
			for y = 4, 10 do
				carnages.dropBarricade(m, 27, y)
				carnages.dropBarricade(m, 30, y)
				carnages.dropBarricade(m, 33, y)
			end
		end
		if m == 3017 then
			for x = 16, 24 do
	...
	removeCircleBarricade = function()
		...
		if m == 3016 then
			for x = 10, 16 do
				carnages.deleteBarricade(m, x, 8)
				carnages.deleteBarricade(m, x, 11)
				carnages.deleteBarricade(m, x, 14)
			end
			for y = 8, 14 do
				carnages.deleteBarricade(m, 10, y)
				carnages.deleteBarricade(m, 13, y)
				carnages.deleteBarricade(m, 16, y)
			end
		end
		if m == 3018 then
			for x = 27, 33 do
				carnages.deleteBarricade(m, x, 4)
				carnages.deleteBarricade(m, x, 7)
				carnages.deleteBarricade(m, x, 10)
			end
			for y = 4, 10 do
				carnages.deleteBarricade(m, 27, y)
				carnages.deleteBarricade(m, 30, y)
				carnages.deleteBarricade(m, 33, y)
			end
		end
		if m == 3017 then
			for x = 16, 24 do
	...
	startCircle = function()
		...
		if m == 3015 then
			carnages.deleteBarricade(m, 8, 3)
			carnages.deleteBarricade(m, 8, 4)
			carnages.deleteBarricade(m, 8, 5)
			carnages.deleteBarricade(m, 8, 6)
			carnages.deleteBarricade(m, 3, 8)
			carnages.deleteBarricade(m, 4, 8)
			carnages.deleteBarricade(m, 5, 8)

			carnages.deleteBarricade(m, 27, 23)
			carnages.deleteBarricade(m, 28, 23)
			carnages.deleteBarricade(m, 29, 23)
			carnages.deleteBarricade(m, 23, 27)
			carnages.deleteBarricade(m, 23, 26)
			carnages.deleteBarricade(m, 23, 25)
		end
	end,
	setBarricade = function()
		...
		if m == 3018 then
			--top left
			for x = 1, 9 do
				carnages.dropBarricade(m, x, 9)
			end
			for y = 1, 9 do
				carnages.dropBarricade(m, 9, y)
			end
		...
		if m == 3015 then
			carnages.dropBarricade(m, 8, 3)
			carnages.dropBarricade(m, 8, 4)
			carnages.dropBarricade(m, 8, 5)
			carnages.dropBarricade(m, 8, 6)
			carnages.dropBarricade(m, 3, 8)
			carnages.dropBarricade(m, 4, 8)
			carnages.dropBarricade(m, 5, 8)

			carnages.dropBarricade(m, 27, 23)
			carnages.dropBarricade(m, 28, 23)
			carnages.dropBarricade(m, 29, 23)
			carnages.dropBarricade(m, 23, 27)
			carnages.dropBarricade(m, 23, 26)
			carnages.dropBarricade(m, 23, 25)
		end
		if m == 3017 then
			for x = 2, 9 do
	...
	removeBarricade = function()
		...
		if m == 3018 then
			--top left
			for x = 1, 9 do
				carnages.deleteBarricade(m, x, 9)
			end
			for y = 1, 9 do
				carnages.deleteBarricade(m, 9, y)
			end
			...
			--bot right
			for x = 30, 38 do
				carnages.deleteBarricade(m, x, 30)
			end
			for y = 30, 38 do
				carnages.deleteBarricade(m, 30, y)
			end
		...
		if m == 3016 then
			...
			for y = 2, 7 do
				carnages.deleteBarricade(m, 6, y)
			end
			...
			for y = 24, 29 do
				carnages.deleteBarricade(m, 25, y)
			end
