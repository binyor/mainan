BomberWarNpc = {
			...
			if #pc > 0 then
				for i = 1, #pc do
					...
					if pc[i].registry["bomber_war_team"] == core.gameRegistry["bomber_war_winner"] then
						...
						if (not player:hasSpace("event_token", 3)) then
							player:sendMinitext("You miss out on your tokens, you had no room!")
						else
							player:addItem("event_token",3)
						end
						pc[i]:sendAnimation(2)
					else
						if (not player:hasSpace("event_token", 1)) then
							player:sendMinitext("You miss out on your tokens, you had no room!")
						else
					...
					pc[i]:warp(31, math.random(6, 15), math.random(11, 13))
	...
	enterArena = function(player)
		local map = 15050
		local t2x = math.random(1,8)
		local y = math.random(4, 37)
		local t1x = math.random(26,33)
		local x = math.random(1,33)
		local pc = core:getObjectsInCell(map, x, y, BL_PC)
		local nearbypc
		local nearbypcfound = {}

		if player.registry["bomber_war_team"] == 1 then
			if getPass(map, t1x, y) == 0 then
				if getObject(map, t1x, y) == 0 then
					if #pc == 0 then
						player.paralyzed = true
						player:warp(map, t1x, y)
					else
		...
		elseif player.registry["bomber_war_team"] == 2 then
			if getPass(map, t2x, y) == 0 then
				if getObject(map, t2x, y) == 0 then
					if #pc == 0 then
						player.paralyzed = true
						player:warp(map, t2x, y)
					else
						return BomberWarNpc.enterArena(player)
					end
				else
