...
InstanceTestNpc = {
		...
		if (#instances > 0) then
			...
			for i = 1, #instances do
				...
				elseif instances[i].special == true then
					...
					if instances[i].map_type_yname == "knight_quest_blank" then
                        knight_instance_quest.quest(
                            instances[i].maps,
                            instances[i].owner
                        )
                    end
	...
	createInstance = function(player, npc, special, displayname, maxsize)
		...
		for z = 1, #instance_sets[instance_set_index].maps do
			...
			if (instance_sets[instance_set_index].maps[z] == 752) then
                mapFile = "../../rtkmaps//Accepted/HubCity.map"
            else 
                mapFile = "../../rtkmaps//Accepted/TK" .. map .. ".map"
            end
