...
int intif_auth(int fd) {
	...
	if ((strcmp((char *)RFIFOP(fd, 5), login_id)) && (strcmp((char *)RFIFOP(fd, 37), login_pw))) {
		WFIFOHEAD(fd, 3);
		WFIFOW(fd, 0) = 0x1000;
		WFIFOB(fd, 2) = 0x01;
		WFIFOSET(fd, 3);
...
////////// UPDATE IP ADDRESS IN DATABASE SO WE MAY TRACK USERS WHILE THEY ARE PLAYING.... ADDED 06-29-2017 ////////////////////////
		...
		strcpy((char *)WFIFOP(RFIFOW(fd, 2), 13), "NexonInc.");
		WFIFOB(RFIFOW(fd, 2), 22) = strlen(thing);
		strcpy((char *)WFIFOP(RFIFOW(fd, 2), 23), thing);
