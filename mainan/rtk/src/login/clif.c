...
int clif_parse(int fd) {
	...
	case 0x00:
		...
		if (ver == nex_version) {
			...
			WFIFOB(fd, 10) = 0x09;
			strcpy((char *)WFIFOP(fd, 11), "NexonInc."); //NexonInc.
			//set_packet_indexes(WFIFOP(fd, 0));
			WFIFOSET(fd, 20);
		}
		else {
			...
			WFIFOB(fd, 7) = 1;
			WFIFOB(fd, 8) = 0x23;
			strcpy((char *)WFIFOP(fd, 9), "http://files.kru.com/NexusTK/patch/");
			set_packet_indexes(WFIFOP(fd, 0));
			WFIFOSET(fd, 44 + 3);
			//session[fd]->eof=1;
		}
	...
	case 0x02:
		...
		if ((RFIFOB(fd, 5) > 12) || (RFIFOB(fd, 5) < 3) || string_check_allchars((char *)RFIFOP(fd, 6), RFIFOB(fd, 5))) {
			clif_message(fd, 0x03, login_msg[LGN_ERRUSER]);
			break;
		}

		if ((RFIFOB(fd, 6 + RFIFOB(fd, 5)) > 8) || (RFIFOB(fd, 6 + RFIFOB(fd, 5)) < 3) || string_check((char *)RFIFOP(fd, 7 + RFIFOB(fd, 5)), RFIFOB(fd, 6 + RFIFOB(fd, 5)))) {
	...
	case 0x03:
		...
		if (string_check((char *)RFIFOP(fd, 7 + RFIFOB(fd, 5)), RFIFOB(fd, 6 + RFIFOB(fd, 5)))) {
			clif_message(fd, 0x05, login_msg[LGN_ERRPASS]);
			break;
		}
	...
	case 0x26:
		...
		if (string_check((char *)RFIFOP(fd, 7 + RFIFOB(fd, 5)), RFIFOB(fd, 6 + RFIFOB(fd, 5))) || string_check((char *)RFIFOP(fd, 8 + RFIFOB(fd, 5) + RFIFOB(fd, 6 + RFIFOB(fd, 5))), RFIFOB(fd, 7 + RFIFOB(fd, 5) + RFIFOB(fd, 6 + RFIFOB(fd, 5))))) {
			clif_message(fd, 0x05, login_msg[LGN_ERRPASS]);
			break;
		}
		if (char_fd) {
