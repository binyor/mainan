...
int map_src_add(const char* r1) {
	...
	if (sscanf(r1, "%d,%[^,],%hd,%d,%d,%hu,%hu,%u,%s,%s,%s,%s,%s, %s,%s", &map_id, map_title, &map_bgm, &pvp, &spell, &light, &weather, &sweeptime, &cantalk, &showghosts, &region, &indoor, &warpout, &bind, map_file) < 13) {
		return -1;
	}
	//[^,],[^\r\n]
...
int nmail_sendmail(USER* sd, const char* to_user, const char* topic, const char* message) {
