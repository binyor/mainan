	...
	for (x = 0; x < MAX_MAGIC_TIMERS; x++) {
		...
		if (sd->status.dura_aether[x].id > 0) {
			...
			if (sd->status.dura_aether[x].duration > 0) {
				...
				if (sd->status.dura_aether[x].duration <= 0) {
					sd->status.dura_aether[x].duration = 0;
                     clif_send_duration(sd, sd->status.dura_aether[x].id, 0, map_id2sd(sd->status.dura_aether[x].caster_id));
			...
			if (sd->status.dura_aether[x].aether > 0) {
				...
				if (sd->status.dura_aether[x].aether <= 0) {
                    // TODO - Originally was &sd->bl which doesn't make sense
...
int pc_givexp(USER* sd, int exp, unsigned int xprate) {
	//int xp_rate=1;
	...
	if (stack > 1) {
		len = sprintf((char *)xpstring, "You cannot gain experience while on top of other players.");
		clif_sendminitext(sd, (char *)xpstring);
		return 0;
	}

	//afk check
	if (sd->afk == 1) {
		len = sprintf((char *)xpstring, "You cannot gain experience while AFK.");
		clif_sendminitext(sd, (char *)xpstring);
		return 0;
	}
	...
	len = sprintf((char *)xpstring, "%u experience!", defaultxp);
...
int pc_warp(USER* sd, int m, int x, int y) {
	...
	if (!map_isloaded(m)) {
		...
		if (x < 0 || x > 255) x = 1; // Just for Justin
		if (y < 0 || y > 255) y = 1;
