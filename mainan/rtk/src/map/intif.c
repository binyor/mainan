...
int intif_savequit(USER* sd) {
	...
	retval = compress2((Bytef *)WFIFOP(char_fd, 6), &clen, (Bytef *)&sd->status, ulen, 1); //fastest compression.
...
int intif_save(USER* sd) {
	...
	WFIFOHEAD(char_fd, clen + 6);
	retval = compress2(WFIFOP(char_fd, 6), &clen, (Bytef *)&sd->status, ulen, 1); //fastest compression.
...
int intif_parse_charload(int fd) {
	unsigned long ulen, clen, retval, x;
	Bytef* cbuf = NULL;
...
int intif_parse_showpostresponse(int fd) {
	...
	strcpy((char *)WFIFOP(header.fd, 10), boarddb_name(header.board));
	...
	WFIFOSET(header.fd, tk_encrypt(header.fd));
	sd->bcount++;
...
int intif_parse_userlist(int fd) {
	...
	for (i = 0; i < count; i++) {
		...
		strcpy((char *)WFIFOP(sd->fd, len + 5), name);
