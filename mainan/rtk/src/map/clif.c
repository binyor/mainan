...
int flags[16] = { 1,2,4,8,16,32,64,128,256,512,1024,2048,4096,8192,16386,32768 };
...
int clif_sendurl(USER* sd, int type, const char* url)
{
	...
	WFIFOSET(sd->fd, tk_encrypt(sd->fd));
...
int clif_popup(USER* sd, const char* buf) {
	...
	strcpy((char *)WFIFOP(sd->fd, 8), buf);
	WFIFOSET(sd->fd, tk_encrypt(sd->fd));
...
int clif_paperpopup(USER* sd, const char* buf, int width, int height) {
	...
	strcpy((char *)WFIFOP(sd->fd, 11), buf); //message
	WFIFOSET(sd->fd, tk_encrypt(sd->fd));
...
int clif_transfer(USER* sd, int serverid, int m, int x, int y) {
	...
	strcpy((char *)WFIFOP(sd->fd, 13), "NexonInc.");
	len = 11;
	WFIFOB(sd->fd, len + 11) = strlen(sd->status.name);
	strcpy((char *)WFIFOP(sd->fd, len + 12), sd->status.name);
...
int clif_transfer_test(USER* sd, int m, int x, int y) {
	...
	strcpy((char *)WFIFOP(sd->fd, len + 12), "Peter");
	len += strlen("Peter") + 1;
	len += 4;
...
int clif_sendBoardQuestionaire(USER* sd, struct board_questionaire q[], int count) {
	...
	for (int i = 0; i < count; i++) {
		...
		len += 1;
		strcpy((char *)WFIFOP(sd->fd, len), q[i].header);
		...
		len += 1;
		strcpy((char *)WFIFOP(sd->fd, len), q[i].question);
...
int clif_closeit(USER* sd) {
	...
	//len=strlen(sd->status.name);
	WFIFOW(sd->fd,11)=SWAP16(9);
	strcpy(WFIFOP(sd->fd,13),"NexonInc.");
	len=11;
	WFIFOB(sd->fd,len+11)=strlen(sd->status.name);
...
int clif_broadcast_sub(struct block_list* bl, va_list ap) {
	...
	strcpy((char *)WFIFOP(sd->fd, 8), msg);
...
int clif_inventory_popup(USER *sd, int slot, const char *body) {
    WFIFOHEAD(sd->fd, strlen(body) + 7);
    WFIFOB(sd->fd, 0) = 0xAA;
    WFIFOW(sd->fd, 1) = SWAP16(strlen(body) + 7);
    WFIFOB(sd->fd, 3) = 0x59;
    WFIFOB(sd->fd, 4) = 0x03;
    WFIFOW(sd->fd, 5) = SWAP16(slot);
    WFIFOW(sd->fd, 7) = SWAP16(strlen(body));
    strcpy((char *)WFIFOP(sd->fd, 9), body);
    WFIFOSET(sd->fd, tk_encrypt(sd->fd));
...
int clif_sendtowns(USER* sd) {
	...
	//	return 0;
	//}
	//AA 00 0C 66 9C 00 34 00 01 01 01 01 00 00 00 
    int slot = RFIFOB(sd->fd, 10) -1;
   	unsigned int itemid =  sd->status.inventory[slot].id;
	char buf[255];
	
	sprintf(buf, "%s", itemdb_name(itemid));
	printf("Slot: %i \n ItemID: %s \n", slot, buf);
	sd->invslot = slot;
    sl_doscript_blargs("onInventoryClick", NULL, 1, &sd->bl);
...
int clif_send_mobbars(struct block_list* bl, va_list ap) {
    ...
    if (!(sd = va_arg(ap, USER*))) return 0;
    if (!(mob = (MOB*)bl)) return 0;
...
int clif_mob_look_close_func(struct block_list* bl, va_list ap) {
	...
	WFIFOW(sd->fd, 7) = SWAP16(sd->mob_count);
...
/*int clif_object_look_sub(struct block_list *bl, va_list ap) {
	...
	case BL_MOB:
		...
		if (mob->data->isnpc == 0) {
			...
			WFIFOB(sd->fd,len+21)=nlen/4;
			WFIFOB(sd->fd,len+22+nlen)=0; //pass flag
			sd->mob_len+=15+nlen;
		} else if (mob->data->isnpc == 1) {
			WFIFOL(sd->fd,len+11)=SWAP32(mob->bl.id);
			WFIFOW(sd->fd,len+15)=SWAP16(32768 + mob->look);
			WFIFOB(sd->fd,len+17)=mob->look_color;
			WFIFOB(sd->fd,len+18)=mob->side;
...
int clif_object_look_specific(USER* sd, unsigned int id) {
	...
	case BL_NPC:
		...
		WFIFOW(sd->fd,15)=SWAP16(32768+b->graphic_id);
		WFIFOB(sd->fd,17)=b->graphic_color;
		WFIFOB(sd->fd,18)=2;//Looking down
		WFIFOW(sd->fd,19)=0;
		WFIFOB(sd->fd,21)=0;
		//WFIFOB(sd->fd,22)=0;
		//sd->mob_=16;
...
int clif_send_duration(USER* sd, int id, int time, USER* tsd) {
	...
	if (id == 0) {
		strcpy((char *)WFIFOP(sd->fd, 6), "Shield");
	}
	else if (tsd != NULL) {
		char buf[len];
		sprintf(buf, "%s (%s)", name, tsd->status.name);
		strcpy((char *)WFIFOP(sd->fd, 6), buf);
	}
	else {
		strcpy((char *)WFIFOP(sd->fd, 6), name);
	}
...
int clif_send_mob_health_sub(struct block_list* bl, va_list ap) {
	...
	WFIFOSET(tsd->fd, tk_encrypt(tsd->fd));
	return 0;
}
...
int clif_send_sub(struct block_list* bl, va_list ap) {
	...
	if (RBUFB(buf, 3) == 0x0D && RBUFB(buf, 5) >= 10) {
		if (pc_readglobalreg(sd, "chann_en") >= 1 && RBUFB(buf, 5) == 10) {
			...
			if (sd) WFIFOSET(sd->fd, tk_encrypt(sd->fd));
			WBUFB(buf, 5) = 10;
		}
...
int clif_mystaytus(USER* sd) {
	...
	if (sd->status.clan == 0) {
		...
		strcpy((char *)WFIFOP(sd->fd, 9), clandb_name(sd->status.clan));
	...
	if (strlen(sd->status.clan_title)) {
		...
		strcpy((char *)WFIFOP(sd->fd, len + 9), sd->status.clan_title);
	...
	if (strlen(sd->status.title)) {
		...
		strcpy((char *)WFIFOP(sd->fd, len + 9), sd->status.title);
	...
	if(sd->group_count==0) {
		...
		USER *tsd = NULL;
		count=0;
        // This is a noop - not sure if fixing it will break anything
		...
		for (x = 0, n = 0, w = 0, r = 0, m = 0, p = 0, a = 0, g = 0; (n + w + r + m + p + a + g) < sd->group_count; x++) {
			...
			switch(classdb_path(tsd->status.class)) {
				case 0:
					sprintf(buf, "%s (N)", tsd->status.name);
					break;
				case 1:
					sprintf(buf, "%s (W)", tsd->status.name);
					break;
				case 2:
					sprintf(buf, "%s (R)", tsd->status.name);
					break;
				case 3:
					sprintf(buf, "%s (M)", tsd->status.name);
					break;
				case 4:
					sprintf(buf, "%s (P)", tsd->status.name);
					break;
				case 5:
					sprintf(buf, "%s (GM)", tsd->status.name);
					break;
				default:
					sprintf(buf, "%s (GM)", tsd->status.name);
					break;
	...
	if (class_name) {
		...
		strcpy((char *)WFIFOP(sd->fd, len + 9), class_name);
	...
	strcpy((char *)WFIFOP(sd->fd,len + 9), equip_status);
	len += equip_len + 1;
	...
	for (x = 0; x < MAX_LEGENDS; x++) {
		if (strlen(sd->status.legends[x].text) && strlen(sd->status.legends[x].name)) {
			...
			if (sd->status.legends[x].tchaid > 0) {
				...
				char* buff = replace_str((char *)sd->status.legends[x].text, "$player", name);
				WFIFOB(sd->fd, len + 10) = strlen(buff);
				strcpy((char *)WFIFOP(sd->fd, len + 11), buff);
				len += strlen(buff) + 3;
			}
			else {
				WFIFOB(sd->fd, len + 10) = strlen(sd->status.legends[x].text);
				strcpy((char *)WFIFOP(sd->fd, len + 11), sd->status.legends[x].text);
...
int clif_cnpclook_sub(struct block_list* bl, va_list ap) {
	...
	if (nd->state != 2) {
		WFIFOB(sd->fd, 59) = len;
		strcpy((char *)WFIFOP(sd->fd, 60), nd->npc_name);
	}
	else {
		WFIFOB(sd->fd, 59) = 0;
		len = 1;
	...
	if (nd->clone) {
		...
		if (strcmpi(nd->gfx.name, "")) {
			WFIFOB(sd->fd, 59) = len;
			strcpy((char *)WFIFOP(sd->fd, 60), nd->gfx.name);
		}
		else {
			WFIFOW(sd->fd, 59) = 0;
			len = 1;
...
int clif_cmoblook_sub(struct block_list* bl, va_list ap) {
	...
	if (mob->state != 2) {
		WFIFOB(sd->fd, 59) = len;
		strcpy((char *)WFIFOP(sd->fd, 60), mob->data->name);
	}
	else {
		WFIFOB(sd->fd, 59) = 0;
		len = 1;
	...
	if (mob->clone) {
		...
		if (strcmpi(mob->gfx.name, "")) {
			WFIFOB(sd->fd, 59) = len;
			strcpy((char *)WFIFOP(sd->fd, 60), mob->gfx.name);
		}
		else {
			WFIFOW(sd->fd, 59) = 0;
			len = 1;
...
// Not sure if anything else beyond here belongs there.
		...
		if((sd->status.state!=2) && (sd->status.state!=5)) {
			...
			strcpy((char *)WFIFOP(src_sd->fd, 28), buf);
		} else {
	...
	WFIFOSET(src_sd->fd, tk_encrypt(src_sd->fd));
...
int clif_sendstatus(USER* sd, int flags) {
	...
	//23,24:26->32 = flag status
		WFIFOB(sd->fd, 6) = sd->status.country;
		WFIFOB(sd->fd, 7) = sd->status.totem;
		WFIFOB(sd->fd, 8) = 0; //idk what this is - maze
		...
		WFIFOB(sd->fd, 31) = 0;
		WFIFOB(sd->fd, 32) = 0;
		WFIFOB(sd->fd, 33) = sd->status.maxinv;
		len += 23;
	}
	...
	WFIFOB(sd->fd, len+8) = 0; //chat toggle?
	WFIFOB(sd->fd, len+9) = 0;
	WFIFOB(sd->fd, len+10) = 0;
	WFIFOB(sd->fd, len+11) = sd->flags; //messages/parcel stuffs
	WFIFOB(sd->fd, len+12) = 0; //idk??????
	...
	if (zx || zy || zm) {
		if ((sd->status.level < map[zm].reqlvl || (sd->status.basehp < map[zm].reqvita && sd->status.basemp < map[zm].reqmana) || sd->status.mark < map[zm].reqmark || (map[zm].reqpath > 0 && sd->status.class != map[zm].reqpath)) && sd->status.gm_level == 0) {
			...
			if (strcmpi(map[zm].maprejectmsg, "") == 0) {
                int level_diff = map[zm].reqlvl - sd->status.level;
				if (abs(level_diff) >= 10) { clif_sendminitext(sd, "Nightmarish visions of your own death repel you."); }
				else if (abs(level_diff) >= 5 && map[zm].reqlvl - sd->status.level < 10) { clif_sendminitext(sd, "You're not quite ready to enter yet."); }
				else if (abs(level_diff) < 5) { clif_sendminitext(sd, "You almost understand the secrets to this entrance."); }
...
int clif_sendRewardInfo(USER* sd, int fd) {
	...
	for (i = 0; i < rewardranks; i++) {
		...
		pos += 1;
		strncpy((char *)WFIFOP(fd, pos), buf, strlen(buf));
		pos += strlen(buf);
...
int clif_parseranking(USER* sd, int fd) {
    ...
    WFIFOB(fd, 7) = getevents(fd); // number of events  * affirmed
    int chosen_event = RFIFOB(fd, 7);

    updateRanks(chosen_event, fd);
    ...
    int totalscores = gettotalscores(chosen_event, fd);
    pos++;
	...
	void dateevent_block(int pos, int eventid, int fd, USER *sd) {
		...
		WFIFOB(fd, pos + 20) = checkevent_claim(eventid, fd, sd);  // Envelope.  0 = new, 1 = read/unclaimed, 2 = no reward -- enables/disables the GetReward button
	}

	void filler_block(int pos, int eventid, int fd, USER *sd) {
		int player_score = checkPlayerScore(eventid, fd, sd);
		int player_rank = checkPlayerRank(eventid, fd, sd);
...
int clif_retrwisp(USER* sd, char* dstname, char* msg) {
	...
	sprintf((char *)buf, "%s> %s", dstname, msg);
...
int clif_sendadditem(USER* sd, int num) {
	...
	strcpy((char *)WFIFOP(sd->fd, len + 1), itemdb_name(id));
	...
	if (sd->status.inventory[num].owner) {
		...
		strcpy((char *)WFIFOP(sd->fd, len + 1), owner);
		len += strlen(owner) + 1;
		FREE(owner);
	}
	else {
...
int clif_equipit(USER* sd, int id) {
	...
	strcpy((char *)WFIFOP(sd->fd, 10), nameof);
	len += strlen(nameof) + 1;

	WFIFOB(sd->fd, len + 9) = strlen(itemdb_name(sd->status.equip[id].id));
	strcpy((char *)WFIFOP(sd->fd, len + 10), itemdb_name(sd->status.equip[id].id));
			...
			if (canwhisper(sd, dst_sd)) {
				if (dst_sd->afk && strcmp(dst_sd->afkmessage, "") != 0) {
					...
					if (!sd->status.gm_level && (dst_sd->optFlags & optFlag_stealth)) {
						...
						clif_retrwisp(sd, dst_sd->status.name, (char *)dst_sd->afkmessage);
					}
				}
				else {
					sl_doscript_strings("characterLog", "whisperLog", 3, dst_sd->status.name, sd->status.name, msg);
...
int clif_speak(struct block_list* bl, va_list ap) {
	...
	WFIFOB(sd->fd, 10) = len;
	len = len + 8;
	strcpy((char *)WFIFOP(sd->fd, 11), msg);
...
int clif_sendanimation_xy(struct block_list* bl, va_list ap) {
	...
	WFIFOB(src->fd, 3) = 41;
	WFIFOL(src->fd, 5) = 0;
	WFIFOW(src->fd, 9) = anim;
	WFIFOW(src->fd, 11) = times;
	WFIFOW(src->fd, 13) = x;
	WFIFOW(src->fd, 15) = y;
	WFIFOSET(src->fd, tk_encrypt(src->fd));
	return 0;
}
...
int clif_sendmagic(USER* sd, int pos) {
	...
	len = strlen(name);
	strcpy((char *)WFIFOP(sd->fd, 8), name);
	WFIFOB(sd->fd, len + 8) = strlen(question);
	strcpy((char *)WFIFOP(sd->fd, len + 9), question);
...
int clif_scriptmes(USER* sd, int id, const char* msg, int previous, int next) {
	...
	if (type == 0) {
		...
		strcpy((char *)WFIFOP(sd->fd, 28), msg);
		...
		strcpy((char *)WFIFOP(sd->fd, 66), msg);
...
int clif_scriptmenu(USER* sd, int id, char* dialog, char* menu[], int size) { // this function does not appear to get called any where at all
	...
	if (type == 0) {
		...
		strcpy((char *)WFIFOP(sd->fd, 22), dialog);
		WFIFOB(sd->fd, strlen(dialog) + 22) = size;
		len = strlen(dialog);

		for (x = 1; x < size + 1; x++) {
			WFIFOB(sd->fd, len + 23) = strlen(menu[x]);
			strcpy((char *)WFIFOP(sd->fd, len + 24), menu[x]);
			len += strlen(menu[x]) + 1;
		...
		strcpy((char *)WFIFOP(sd->fd, 60), dialog);
		WFIFOB(sd->fd, strlen(dialog) + 60) = size;
		len = strlen(dialog);

		for (x = 1; x < size + 1; x++) {
			WFIFOB(sd->fd, len + 61) = strlen(menu[x]);
			strcpy((char *)WFIFOP(sd->fd, len + 62), menu[x]);
			len += strlen(menu[x]) + 1;
...
int clif_scriptmenuseq(USER* sd, int id, const char* dialog, const char* menu[], int size, int previous, int next) {
	int graphic_id = sd->npc_g;
	int color = sd->npc_gc;
	int x;
	int len = 0;
	...
	if (type == 0) {
		...
		strcpy((char *)WFIFOP(sd->fd, 28), dialog);
		len = strlen(dialog) + 1;
		WFIFOB(sd->fd, len + 27) = size;
		len += 1;
		for (x = 1; x < size + 1; x++) {
			WFIFOB(sd->fd, len + 27) = strlen(menu[x]);
			strcpy((char *)WFIFOP(sd->fd, len + 28), menu[x]);
			len += strlen(menu[x]) + 1;
		}
		...
		strcpy((char *)WFIFOP(sd->fd, 68), dialog);
		len = strlen(dialog) + 68;
		WFIFOB(sd->fd, len) = size;
		len += 1;
		for (x = 1; x < size + 1; x++) {
			WFIFOB(sd->fd, len) = strlen(menu[x]);
			strcpy((char *)WFIFOP(sd->fd, len + 1), menu[x]);
			len += strlen(menu[x]) + 1;
		}
...
int clif_inputseq(USER* sd, int id, const char* dialog, const char* dialog2, const char* dialog3, const char* menu[], int size, int previous, int next) {
	...
	strcpy((char *)WFIFOP(sd->fd, len + 1), dialog2);
	len += strlen(dialog2) + 1;

	WFIFOB(sd->fd, len) = 42;
	len += 1;
	WFIFOB(sd->fd, len) = strlen(dialog3);
	strcpy((char *)WFIFOP(sd->fd, len + 1), dialog3);
...
int clif_parseattack(USER* sd) {
	...
	if (itemdb_look(sd->status.equip[EQ_WEAP].id) >= 20000 && itemdb_look(sd->status.equip[EQ_WEAP].id) < 30000) { // bows
		sl_doscript_blargs(itemdb_yname(sd->status.equip[EQ_WEAP].id), "shootArrow", 1, &sd->bl);
...
int clif_parsefriends(USER* sd, unsigned char* friendList, int len) {
	int i = 0;
	int j = 0;
	char friends[20][16];
	char escape[16];
...
unsigned int metacrc(char* file) {
	...
	fclose(fp);
	checksum = crc32(checksum, (Bytef *)fileinf, size);
...
int send_metafile(USER* sd, char* file) {
	...
	fseek(fp, 0, SEEK_SET);
	//CALLOC(ubuf,0,ulen);
	ubuf = (Bytef*)calloc(ulen + 1, sizeof(Bytef));
	clen = compressBound(ulen);
	cbuf = (Bytef*)calloc(clen + 1, sizeof(Bytef));
	...
	strcpy((char *)WFIFOP(sd->fd, 7), file);
	len += strlen(file) + 1;
...
int clif_updatestate(struct block_list *bl, va_list ap) {
	...
	if(sd->status.state==4) {
		...
		len+=strlen(sd->status.name)+1;
		strcpy((char *)WFIFOP(src_sd->fd,17),buf);
	...
	//	}*/
		...
		if((sd->status.state!=5) && (sd->status.state!=2)) {
			...
			strcpy((char *)WFIFOP(src_sd->fd, 23), buf);
		} else {
...
int clif_showboards(USER* sd) {
	...
	WFIFOB(sd->fd, 5) = 1;
	WFIFOB(sd->fd, 6) = 13;
	strcpy((char *)WFIFOP(sd->fd, 7), "NexusTKBoards");
	len = 15;
	b_count = 0;
	for (i = 0; i < 256; i++)
	{
		for (x = 0; x < 256; x++)
		{
			if (boarddb_sort(x) == i && boarddb_level(x) <= sd->status.level && boarddb_gmlevel(x) <= sd->status.gm_level
				...
				strcpy((char *)WFIFOP(sd->fd, len + 9), boarddb_name(x));
...
int clif_buydialog(USER* sd, unsigned int id, const char* dialog, struct item* item, int price[], int count) {
	...
	if (graphic > 0) {
		...
		for (x = 0; x < count; x++) {
			...
			strcpy((char *)WFIFOP(sd->fd, len + 23), name);
			len += strlen(name) + 1;

			if (strcmp((char *)itemdb_buytext(item[x].id), "") != 0) {
				strcpy(buff, (char *)itemdb_buytext(item[x].id));
			}
		...
		strcpy((char *)WFIFOP(sd->fd, 62), dialog);
		len = strlen(dialog);
		...
		for (x = 0; x < count; x++) {
			...
			strcpy((char *)WFIFOP(sd->fd, len + 63), name);
			len += strlen(name) + 1;
...
int clif_input(USER* sd, int id, const char* dialog, const char* item) {
	...
	if (type == 0) {
		...
		len += 1;
		strcpy((char *)WFIFOP(sd->fd, len + 23), item);
		len += strlen(item) + 1;
		...
		len += 1;
		strcpy((char *)WFIFOP(sd->fd, len + 61), item);
		len += strlen(item) + 1;
...
int clif_clickonplayer(USER* sd, struct block_list* bl) {
	...
	if (strlen(tsd->status.title) > 0) {
		...
		strcpy((char *)WFIFOP(sd->fd, 6), tsd->status.title);
	...
	if (tsd->status.clan > 0) {
		...
		strcpy((char *)WFIFOP(sd->fd, len + 6), clandb_name(tsd->status.clan));
	...
	if (strlen(tsd->status.clan_title) > 0) {
		...
		strcpy((char *)WFIFOP(sd->fd, len + 6), tsd->status.clan_title);
	...
	if (classdb_name(tsd->status.class, tsd->status.mark)) {
		...
		strcpy((char *)WFIFOP(sd->fd, len + 6), classdb_name(tsd->status.class, tsd->status.mark));
	...
	strcpy((char *)WFIFOP(sd->fd, len + 6), tsd->status.name);
	...
	strcpy((char *)WFIFOP(sd->fd,len + 6),equip_status);
	len += equip_len + 1;
	...
	for (x = 0; x < MAX_LEGENDS; x++) {
		if (strlen(tsd->status.legends[x].text) && strlen(tsd->status.legends[x].name)) {
			...
			if (tsd->status.legends[x].tchaid > 0) {
				...
				char* buff = replace_str((char *)tsd->status.legends[x].text, "$player", name);
...
int clif_groupstatus(USER* sd) {
	...
	for (x = 0, n = 0, w = 0, r = 0, m = 0, p = 0, g = 0; (n + w + r + m + p + g) < sd->group_count; x++) {
		...
		strcpy((char *)WFIFOP(sd->fd, len + 12), buf);
...
int clif_grouphealth_update(USER* sd) {
	...
	*/
	...
	for (x = 0; x < sd->group_count; x++) {
		...
		strcpy((char *)WFIFOP(sd->fd, 11), buf);
...
int clif_startexchange(USER* sd, unsigned int target) {
	...
	if (tsd->status.settingFlags & FLAG_EXCHANGE) {
		...
		strcpy((char *)WFIFOP(sd->fd, len + 7), buff);
		len += strlen(buff) + 1;
		...
		strcpy((char *)WFIFOP(tsd->fd, len + 7), buff);
		len += strlen(buff) + 1;
...
int clif_exchange_additem_else(USER* sd, USER* tsd, int id) {
	...
	strcpy((char *)WFIFOP(sd->fd, len + 12), buff);
	len += strlen(buff) + 5;
	...
	strcpy((char *)WFIFOP(tsd->fd, 12), buff);
	len += strlen(buff) + 1;
...
int clif_exchange_message(USER* sd, char* message, int type, int extra) {
	...
	strcpy((char *)WFIFOP(sd->fd, 8), message);
...
int clif_mapselect(USER* sd, const char* wm, int* x0, int* y0, const char** mname, unsigned int* id, int* x1, int* y1, int i) {
	int len = 0;
	int x, y;
	...
	for (x = 0; x < i; x++) {
		...
		strcpy((char *)WFIFOP(sd->fd, len + 6), mname[x]);
...
int clif_pb_sub(struct block_list* bl, va_list ap) {
	...
	if (path == 50 || path == 0)
		...
		strcpy((char *)WFIFOP(sd->fd, len[0] + 19), tsd->status.name);
