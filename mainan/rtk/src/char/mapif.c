...
int mapif_send_newmp(int fd, unsigned int id, unsigned int flags) {
	...
	mapif_send(fd, (unsigned char *)data, 8, ALL); //Send to all servers.
	return 0;
}
int mapif_parse_auth(int fd) {
	...
	if (cmd == 0x3000) {
		if ((strcmp((char *)RFIFOP(fd, 2), char_id)) && (strcmp((char *)RFIFOP(fd, 34), char_pw))) {
			WFIFOHEAD(fd, 4);
			WFIFOW(fd, 0) = 0x3800;
			WFIFOB(fd, 2) = 0x01;
			WFIFOB(fd, 3) = 0x00;
...
int mapif_parse_requestchar(int fd) {
	unsigned long ulen, clen, retval;
	char* cbuf;
...
int mapif_parse_savechar(int fd) {
	...
	retval = uncompress((Bytef *)cbuf, &clen, RFIFOP(fd, 6), ulen);
